...
- 4. Run uvicorn: `uvicorn app.app:app --host 0.0.0.0 --port 80`
- 5. Then make a POST request to localhost:80/macro with an image file of facial skin texture in its body

Reproduce learning process:

1. Download this project
2. Create venv
3. Install all dependencies with `pip install -r requirements-dev.txt`
...
6. Run `dvc repro`
7. To see experiment metrics, run `mlflow ui` and move to browser
